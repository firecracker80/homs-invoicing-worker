...
async function createBookingRecords(tenant, snapshot, gatewayRef) {
  ...
  for (const b of s.securityDeposit.blocks) {
    items.push({
      ...
      "Status": "Pending",
      "Notes": s.securityDeposit.blocks.length > 1
        ? `Depósito de seguridad (reembolsable) — bloque ${b.block}: ${b.nights} nights, tier ${b.tier}`
        : `Depósito de seguridad (reembolsable): ${b.nights} nights, tier ${b.tier}`
    });
  }
  ...
  // Map payment record IDs by purchase unit (Notes carries bookingId-RENT / -DEP)
  const paymentIds = {};
  for (const p of payments) {
    const note = p.fields?.Notes || "";
    if (note.endsWith("-RENT")) paymentIds.RENT = p.id;
    if (note.endsWith("-DEP")) paymentIds.DEP = p.id;
  }
  return { orderRecordId: order.id, paymentIds, itemCount: orderItems.length, paymentCount: payments.length };
}

async function atUpdate(tenant, table, recordId, fields) {
  const res = await fetch(`${API}/${tenant.airtableBaseId}/${encodeURIComponent(table)}/${recordId}`, {
    method: "PATCH",
    headers: { "Authorization": `Bearer ${tenant.airtableToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ fields, typecast: true })
  });
  if (!res.ok) throw new Error(`Airtable update ${table} failed: ${res.status} ${await res.text()}`);
  return (await res.json());
}

// Fallback lookup when the snapshot lacks stored record IDs
async function atFind(tenant, table, formula) {
  const url = `${API}/${tenant.airtableBaseId}/${encodeURIComponent(table)}?filterByFormula=${encodeURIComponent(formula)}&maxRecords=3`;
  const res = await fetch(url, { headers: { "Authorization": `Bearer ${tenant.airtableToken}` } });
  if (!res.ok) throw new Error(`Airtable find ${table} failed: ${res.status} ${await res.text()}`);
  return (await res.json()).records;
}
...
export { createBookingRecords, atCreate, atUpdate, atFind };
